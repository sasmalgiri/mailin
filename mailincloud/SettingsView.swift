...
struct SettingsView: View {
    ...
    // Mail Account
    @AppStorage("imapServer") private var imapServer = ""
    @AppStorage("imapPort") private var imapPort = "993"
    @AppStorage("imapUsername") private var imapUsername = ""
    @AppStorage("smtpServer") private var smtpServer = ""
    @AppStorage("smtpPort") private var smtpPort = "587"
        ...
        Form {
            ...
            Section {
                ...
                TextField("Port", text: $imapPort)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 80)
                TextField("Username / Email", text: $imapUsername)
                    .textFieldStyle(.roundedBorder)
                SecureField("Password / App Password", text: $imapPassword)
                    .textFieldStyle(.roundedBorder)
                    .onAppear { imapPassword = KeychainHelper.load(key: "settings_imapPassword") }
            } header: {
                Text("IMAP (Receive)")
                    .font(.headline)
            } footer: {
            ...
            Section {
                ...
                TextField("Port", text: $smtpPort)
                    .textFieldStyle(.roundedBorder)
                    .frame(width: 80)
                TextField("Username / Email", text: $smtpUsername)
                    .textFieldStyle(.roundedBorder)
                SecureField("Password / App Password", text: $smtpPassword)
                    .textFieldStyle(.roundedBorder)
                    .onAppear { smtpPassword = KeychainHelper.load(key: "settings_smtpPassword") }
            } header: {
                Text("SMTP (Send)")
                    .font(.headline)
            }
            ...
            Section {
                Toggle("Email Signature", isOn: Binding(
                    get: { UserDefaults.standard.bool(forKey: "emailSignatureEnabled") },
                    set: { UserDefaults.standard.set($0, forKey: "emailSignatureEnabled") }
                ))

                if UserDefaults.standard.bool(forKey: "emailSignatureEnabled") {
                    TextEditor(text: Binding(
                        get: { UserDefaults.standard.string(forKey: "emailSignatureText") ?? "" },
                        set: { UserDefaults.standard.set($0, forKey: "emailSignatureText") }
                    ))
                ...
                Text("Appended to outgoing emails after \"-- \" separator.")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }

            Section {
                Toggle("Include X-Mailer header", isOn: Binding(
                    get: { UserDefaults.standard.bool(forKey: "includeXMailerHeader") },
                    set: { UserDefaults.standard.set($0, forKey: "includeXMailerHeader") }
                ))
            } header: {
                Text("Privacy")
                    .font(.headline)
            } footer: {
                Text("When off, outgoing emails won't identify mailin as the sending app.")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            Section {
                Button("Save Account") {
                    KeychainHelper.save(key: "settings_imapPassword", value: imapPassword)
                    KeychainHelper.save(key: "settings_smtpPassword", value: smtpPassword)
                    if !imapUsername.isEmpty && defaultSenderEmail.isEmpty {
                        defaultSenderEmail = imapUsername
                    }
                    accountSaved = true
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) { accountSaved = false }
                }
                .buttonStyle(.borderedProminent)
                .disabled(imapServer.isEmpty && smtpServer.isEmpty)

                if accountSaved {
                    Label("Account settings saved.", systemImage: "checkmark.circle.fill")
                        .font(Typography.caption1)
                        .foregroundColor(.green)
                }
            }
