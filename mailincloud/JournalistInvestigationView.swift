...
struct JournalistInvestigationView: View {
        ...
        return ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let selected = selectedContactEmail {
                    ...
                    let contactEmails = emails.filter {
    ...
    private var overviewView: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 12) {
                    overviewCard("Total Emails", value: "\(emails.count)", icon: "envelope", color: .blue)
                ...
                HStack(spacing: 12) {
                    ...
                    overviewCard("Attachments", value: "\(emails.flatMap { $0.attachments }.count)", icon: "paperclip", color: .brown)
    ...
    private var pdfExportSheet: some View {
        VStack(spacing: 16) {
            HStack {
                ...
                Button("Generate PDF") {
                    Task {
                        let bookmarked = emails.filter { bookmarkedEmailIDs.contains($0.id) }
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("• \(bookmarked > 0 ? "\(bookmarked) bookmarked" : "\(emails.count) total") emails")
    ...
    private func computeAvgPerDay() -> String {
        ...
        return String(format: "%.1f", Double(emails.count) / Double(days))
    }
    ...
    private func generateExportText() -> String {
        ...
        if !bookmarkedEmailIDs.isEmpty {
            ...
            text += String(repeating: "-", count: 40) + "\n"
            for email in emails where bookmarkedEmailIDs.contains(email.id) {
