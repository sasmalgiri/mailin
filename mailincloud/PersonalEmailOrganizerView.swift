...
struct PersonalEmailOrganizerView: View {
    ...
    private func loadV4Data() async {
        ...
        if #available(macOS 26, iOS 26, *) {
            if aiTriageText.isEmpty {
                ...
                let result = try? await FoundationModelEngine.triageStructured(emails.prefix(50).map { $0 })
    ...
    private var overviewView: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                HStack(spacing: 12) {
                    quickStatCard("Total Emails", value: "\(emails.count)", icon: "envelope", color: .blue)
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                    ForEach(categories.sorted(by: { $0.value.count > $1.value.count }), id: \.key) { category, emails in
                        categoryCard(category, count: emails.count)
                    }
                }
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 8) {
                    ...
                    Button {
                        ...
                        VStack(spacing: 4) {
                            ...
                            Text("\(emails.count)").font(.system(size: 14, weight: .bold)).foregroundColor(.blue)
    ...
    private var cleanupView: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                let noSubject = emails.filter { ($0.headers["Subject"] ?? "").isEmpty }
                ...
                let totalBodySize = emails.reduce(0) { $0 + $1.plainBody.count + $1.htmlBody.count }
                let totalAttSize = emails.flatMap { $0.attachments }.reduce(0) { $0 + $1.size }
                HStack(spacing: 20) {
                    VStack {
    ...
    private var topSendersThisMonth: some View {
        ...
        let recentEmails = emails.filter {
    ...
    private var newsletterUnsubscribeSuggestions: some View {
        let newsletters = emails.filter { email in
    ...
    private var uniqueContacts: Set<String> {
        Set(emails.compactMap { extractEmail($0.headers["From"] ?? "").lowercased() }.filter { !$0.isEmpty })
    }

    private var totalAttachments: Int {
        emails.flatMap { $0.attachments }.count
    }
    ...
    private func attachmentTypeStats() -> [(String, Int)] {
        ...
        for att in emails.flatMap({ $0.attachments }) {
    ...
    private func findLargeEmails() -> [MBOXParser.RawEmail] {
        emails.filter { $0.plainBody.count + $0.htmlBody.count > 100_000 }
