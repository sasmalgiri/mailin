...
struct FoundationModelEngine {
    ...
    static func hybridExpertSynthesis(
        ...
        // Layer 1: Run experts + sub-queries in parallel
        ...
        await withTaskGroup(of: (String, Int?, ExpertRole?, AISessionFindings).self) { group in
            ...
            for await (type, idx, role, findings) in group {
                ...
                } else if type == "map", let i = idx, i < subQueries.count {
                    mapIndexed.append((i, subQueries[i], findings))
                }
            }
        ...
        let instructions = """
            ...
            with chunk-level evidence, and \(totalParallel) parallel expert sessions produced \
    ...
    private static func runExpertStructured(
        ...
        case .sentimentExpert:
            expertInstructions = """
                You are a sentiment and emotional intelligence expert. \
                Identify emotional patterns, tone escalation, deteriorating relationships, and mood trends. \
                Flag conversations showing conflict or unusual emotional intensity. \
                Cross-reference sentiment with contacts to find relationship dynamics.
                """
        case .entityExpert:
            expertInstructions = """
                You are a people and relationship analyst. \
                Map communication networks, identify key contacts and their roles, detect cliques or isolated contacts. \
                Note unusual patterns like one-way communication or sudden contact changes. \
                Cross-reference domains to identify organizational affiliations.
                """
        case .topicExpert:
            expertInstructions = """
                You are a content and topic analyst. \
                Identify thematic clusters, emerging discussions, declining topics, and topic evolution over time. \
                Cross-reference topics with senders to find who drives which conversations. \
                Note topic shifts that might indicate changing priorities.
                """
        case .timelineExpert:
            expertInstructions = """
                You are a temporal pattern analyst. \
                Identify communication rhythms, volume anomalies, activity spikes, quiet periods, and behavioral patterns. \
                Flag unusual timing like late-night activity or sudden volume changes. \
                Cross-reference timing with events or topics to find causation.
                """
        case .securityExpert:
            expertInstructions = """
                You are a cybersecurity and data protection analyst. \
                Prioritize actionable threats: phishing attempts, PII exposure, suspicious senders, data leakage risks. \
                Flag one-time domains, unusual recipient patterns, and behavioral anomalies. \
                Rate findings by risk severity and recommend specific actions.
                """
        }
