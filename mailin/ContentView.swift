...
struct ContentView: View {
    ...
    var body: some View {
        ...
        #if !OFFLINE_MODE
        .sheet(isPresented: $appState.showIMAPConfig) {
            IMAPConfigView { fetchedEmails in
                viewModel.appendEmails(fetchedEmails)
            }
            #if os(macOS)
            .frame(minWidth: 500, minHeight: 400)
            #else
        ...
        .sheet(isPresented: $appState.showComposeEmail) {
            ComposeEmailView(mode: .new)
                #if os(macOS)
                .frame(minWidth: 500, minHeight: 400)
                #else
                .presentationDetents([.large])
        ...
        .sheet(isPresented: $appState.showCloudConnect) {
            CloudConnectView { fetchedEmails in
                viewModel.appendEmails(fetchedEmails)
            }
            .environmentObject(storeManager)
            #if os(iOS)
    ...
    private var mainLayout: some View {
        ...
        NavigationSplitView {
            ...
            Group {
                if !modelVM.showParsedList {
                    WelcomeHubView(onOpenArchive: { openPanelFallback() }, onBrowseFiles: { openPanelFallback() }, onLoadSample: { loadSampleData() })
                        .frame(maxWidth: .infinity, maxHeight: .infinity)
                ...
                if modelVM.showParsedList {
                    ...
                    Button {
                        withAnimation(.easeInOut(duration: 0.25)) {
                            appState.inlineComposeMode = .new
                        }
                    } label: {
                        Label("Compose", systemImage: "square.and.pencil")
                    }
                    .help("Compose New Email (⌘N)")

                    Button { appState.showIMAPConfig = true } label: {
                        Label("Fetch Mail", systemImage: "tray.and.arrow.down")
                    }
                    .help("IMAP Fetch")
                    #endif
        ...
        return NavigationStack {
            Group {
                if modelVM.showParsedList {
                    ...
                    WelcomeHubView(onOpenArchive: { openPanelFallback() }, onBrowseFiles: { showFileImporter = true }, onLoadSample: { loadSampleData() })
                }
            }
    ...
    // MARK: - Email Action Bar (persistent header)

    #if os(macOS) && !OFFLINE_MODE
        ...
        VStack(spacing: 0) {
            HStack(spacing: 2) {
                emailActionButton(
                    ...
                    gradient: [Color(red: 0.25, green: 0.48, blue: 1.0), Color(red: 0.40, green: 0.30, blue: 0.95)],
                    isActive: appState.inlineComposeMode != nil
                ) {
                    withAnimation(.easeInOut(duration: 0.25)) {
                        if appState.inlineComposeMode != nil {
                            appState.inlineComposeMode = nil
                        } else {
                            appState.inlineComposeMode = .new
                        }
                    }
                ...
                emailActionButton(
                    ...
                    gradient: [Color(red: 0.20, green: 0.78, blue: 0.45), Color(red: 0.10, green: 0.60, blue: 0.40)],
                    isActive: false
                ) {
                    appState.showIMAPConfig = true
                }
                ...
                emailActionButton(
                    title: "Cloud",
                    icon: "cloud.fill",
                    gradient: [Color(red: 0.58, green: 0.35, blue: 0.92), Color(red: 0.40, green: 0.20, blue: 0.85)],
                    isActive: false
                ) {
                    appState.showCloudConnect = true
                }
                ...
                emailActionButton(
                    title: "Attachments",
                    icon: "paperclip",
                    gradient: [Color(red: 1.0, green: 0.58, blue: 0.0), Color(red: 0.95, green: 0.40, blue: 0.10)],
                    isActive: false
                ) {
                ...
                if !selectedEmailIDs.isEmpty, let selectedID = selectedEmailIDs.first,
                   let email = modelVM.filteredEmails.first(where: { $0.id == selectedID }) {
                    HStack(spacing: 2) {
                        emailActionSmallButton(icon: "arrowshape.turn.up.left.fill", tooltip: "Reply") {
                            withAnimation(.easeInOut(duration: 0.25)) {
                                appState.inlineComposeMode = .reply(original: email)
                            }
                        }
                        emailActionSmallButton(icon: "arrowshape.turn.up.left.2.fill", tooltip: "Reply All") {
                            withAnimation(.easeInOut(duration: 0.25)) {
                                appState.inlineComposeMode = .replyAll(original: email)
                            }
                        }
                        emailActionSmallButton(icon: "arrowshape.turn.up.right.fill", tooltip: "Forward") {
                            withAnimation(.easeInOut(duration: 0.25)) {
                                appState.inlineComposeMode = .forward(original: email)
                            }
                        }
                    }
                    .padding(.horizontal, 4)
    ...
    private func emailActionButton(title: String, icon: String, gradient: [Color], isActive: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 5) {
                ...
                Text(title)
                    ...
                    .foregroundColor(isActive ? .white : AppColors.primary.opacity(0.85))
            }
            .padding(.horizontal, 10)
            .padding(.vertical, 5)
            .background(
    ...
    private func emailActionSmallButton(icon: String, tooltip: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: icon)
    ...
    private var emailActionDivider: some View {
        Rectangle()
    ...
    // MARK: - Inline Compose Panel (Thunderbird-style)

    #if os(macOS) && !OFFLINE_MODE
    private var inlineComposePanel: some View {
        VStack(spacing: 0) {
            HStack {
                Image(systemName: "square.and.pencil")
                    .foregroundColor(AppColors.primary)
                Text(inlineComposeTitle)
            ...
            ComposeEmailView(
                mode: appState.inlineComposeMode ?? .new,
                onClose: {
                    withAnimation(.easeInOut(duration: 0.2)) {
                        appState.inlineComposeMode = nil
                    }
    ...
    private var inlineComposeTitle: String {
        switch appState.inlineComposeMode {
    ...
    private var personaFeatureBadges: [BadgeInfo] {
        ...
        case .forensic:
            return [
                ...
                BadgeInfo(icon: "checkmark.seal.fill", text: "Tamper-Evident", color: .orange),
                BadgeInfo(icon: "doc.badge.gearshape", text: "Forensic Workflow", color: .blue)
            ]
        case .legal:
            return [
                BadgeInfo(icon: "lock.shield.fill", text: "Privilege Review", color: .indigo),
        ...
        case .journalist:
            return [
                ...
                BadgeInfo(icon: "lock.shield.fill", text: "Local-Only Analysis", color: .green)
            ]
        case .personal:
            return [
                BadgeInfo(icon: "gift.fill", text: "Free to Try", color: .green),
                BadgeInfo(icon: "lock.shield.fill", text: "Private by Default", color: .blue),
    ...
    private var personaPrivacyNote: String {
        switch personaManager.selectedPersona {
        case .forensic: return "All processing stays on-device. Cloud AI is automatically blocked in forensic mode to maintain chain of custody."
        case .legal: return "Documents stay on your Mac by default. Privilege review remains local unless Cloud AI is explicitly enabled."
        case .itAdmin: return "Header analysis happens locally by default. No email content is transmitted unless you enable Cloud AI in Settings."
        case .journalist: return "Source material stays on your Mac by default. No data is shared with external servers unless you enable Cloud AI."
        case .personal, .general: return "No data leaves your device unless you explicitly enable Cloud AI in Settings. Default analysis is fully local."
        }
    }
