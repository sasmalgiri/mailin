...
struct EmailDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showComposeReplyAll) {
            ComposeEmailView(mode: .replyAll(original: email))
                #if os(iOS)
                .presentationDetents([.large])
                #endif
        }
        .sheet(isPresented: $showComposeForward) {
            ComposeEmailView(mode: .forward(original: email))
                #if os(iOS)
                .presentationDetents([.large])
                #endif
        }
            ...
            HStack(spacing: 0) {
                ...
                mailActionGroup(
                    actions: [
                        ...
                        ("arrowshape.turn.up.left.2.fill", "Reply All", { showComposeReplyAll = true }),
                        ("arrowshape.turn.up.right.fill", "Forward", { showComposeForward = true }),
                    ]
                )
                ...
                Button { showComposeReplyAll = true } label: {
                    Image(systemName: "arrowshape.turn.up.left.2.fill")
                }
                .buttonStyle(.plain)
                .accessibilityLabel("Reply All")

                Button { showComposeForward = true } label: {
                ...
                Menu {
                    Button { NotificationCenter.default.post(name: .toggleReadCurrentEmail, object: email.id) } label: {
                        Label("Mark Read/Unread", systemImage: "envelope.badge.fill")
                    }
                    Button { NotificationCenter.default.post(name: .togglePinEmail, object: email.id) } label: {
                        Label("Flag", systemImage: "flag.fill")
                    }
                    Button { NotificationCenter.default.post(name: .deleteCurrentEmail, object: email.id) } label: {
                        Label("Delete", systemImage: "trash")
                    }
                    Button { NotificationCenter.default.post(name: .archiveCurrentEmail, object: email.id) } label: {
    ...
    private func mailIconButton(icon: String, tooltip: String, color: Color, action: @escaping () -> Void) -> some View {
    ...
    private var mailActionDivider: some View {
        Rectangle()
