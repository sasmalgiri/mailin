...
class KnowledgeGraph: Codable, @unchecked Sendable {
    ...
    /// Removes a node and every incident edge. Used by `EntityResolver`
    /// after a duplicate node's edges have been redirected to a canonical
    /// counterpart. Safe to call with an unknown id (no-op).
    func removeNode(id: String) {
        guard nodes[id] != nil else { return }
        let edgeIDsToRemove = (adjacency[id] ?? []).union(reverseAdjacency[id] ?? [])
        for edgeID in edgeIDsToRemove {
            if let edge = edges[edgeID] {
                adjacency[edge.sourceID]?.remove(edgeID)
                reverseAdjacency[edge.targetID]?.remove(edgeID)
                edges.removeValue(forKey: edgeID)
            }
        }
        adjacency.removeValue(forKey: id)
        reverseAdjacency.removeValue(forKey: id)
...
struct KnowledgeGraphBuilder {
    ...
    static func build(from emails: [MBOXParser.RawEmail], into graph: KnowledgeGraph) {
        ...
        // Canonicalize duplicate person/organization nodes before persisting.
        // Conservative: requires multiple signals to agree before merging.
        _ = EntityResolver.resolve(graph: graph)
