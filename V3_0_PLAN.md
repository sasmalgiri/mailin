...
| A5 | Import receipt — **rescoped 2026-09-23; verdict DONE** | `ImportReceipt.swift` **already exists** (312 lines): source name/size/SHA-256, parser+version, discovered/parsed/inserted/duplicates/damaged/skipped/persistFailed/indexed, FTS-drift state, resume provenance, self-hash, JSON persistence. Remaining work is therefore *not* the model. (a) The `ImportReconciler` verdict of exactly Complete / **Partial** / Failed is **built and tested** (`ImportVerdict.swift`, 11 tests) and surfaced on `RunSummary`; it is computed from the receipt's counts so it cannot change an existing receipt's content hash. (b) The receipt window (`ImportReceiptView.swift`) is **built**: verdict banner, source identity with SHA-256, full accounting, coverage, problems, tamper-evidence tier, `Recheck` (re-reads the live store and index and reports drift), `Retry` only when the host supplies an action, plus Save and Print. Reachable from File ▸ Last Import Receipt…, reading the durable receipt from disk. (c) printable/saveable output **done** (plain text, filable without this app). Still open: (d) attachment-family coverage, and wiring `Retry` to a real re-run. | 3–4 d (was 6–8) |
...
| S0–S6 | **Size-limit removal — finalized plan in `SIZE_LIMITS_DESIGN.md` §6–§7** | Sourced answer to "can we have no size limits": the per-message 100 MB cap and the PST 50 GB cap are **ours** and removable; the NSF 64 GB cap is **stale** (ODS 53+ allows 256 GB per HCL); ANSI PST's 2 GB is a real format limit; and a constraint nobody had checked — **SQLite's default max BLOB is also its max ROW size at 1 GB**, so a >1 GB message cannot live in `email_bodies.raw` at all and SQLite advises against raising the limit. Design: parse by byte offsets (Mime4J `M_RAW` shape, no body decoding), store large bodies as external content-addressed blobs (also the shape §9's iCloud segments need), index a bounded text prefix and report coverage, replace refusals with preflight plus "not tested above N". Audit found several elements half-built: bodies are already in a separate table, the FTS text budget already exists (but truncates **silently** at 50 K chars), and `BoundedRegexSearch` already establishes the honest-truncation pattern to copy. Order: S0 report the index budget, S1 correct the PST/NSF caps, S2 storage preflight, S3 external blob tier, S4 offset parser, S5 locator-backed reads, S6 executed size tests. | 24–35 d total |
| P3.5 | Per-message parse churn | The remaining candidate for the import peak after four hypotheses were measured and three falsified (`RELEASE_READINESS.md` §P0.2 ledger). Not a retained structure but allocator churn: `currentLines` holds one `String` per line, `joined()` makes a full copy, and `EmailBodyExtractor` re-splits the same text. Parse from `Data`/byte slices instead. Guard rails: the round-trip, recovery-report and I/O-error tests must stay green, and the result must be measured with repeated runs because single-run RSS varies ±15 %. | 8–12 d |
| P3.4 | Per-message parse residency — **closed, hypothesis falsified** | **The measured owner of the import peak** (`RELEASE_READINESS.md` §P0.2 attribution: parse-only accounts for +334.8 MiB of a ~420 MiB peak, while store and index add marginal amounts). For every message the parser joins all lines into one full string copy, MIMEParser builds part objects, attachments are decoded, and `RawEmail` then retains `rawSource` plus `plainBody` plus `htmlBody` — several copies resident simultaneously, with the worst single message setting the high-water mark. Reduce the transient graph: parse from the line buffer without the full join, avoid retaining `rawSource` once the store has it, and use the existing `isBodyCompacted`/`bodyPreview` compaction. Re-measure per format. | 5–8 d |
| P3.3 | Import-time memory budget — **built, but measured to change nothing** | Measured owner of the 400 MiB import peak: `PRAGMA cache_size = -131072` (128 MB) + `mmap_size = 256 MB` on the store, and one SQLite connection per year-shard (19 for a 2007–2025 corpus). Lower the cache/mmap budget for the duration of an import and cap concurrently-open shards (or group batches by year), then re-measure. This — not batch size — is the lever. | 3–5 d |
