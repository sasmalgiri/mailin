...
struct InvestigationReportGenerator {
    ...
    /// Generate a PDF report with optional AI-enhanced analysis.
    /// This async variant attempts to gather AI insights before generating the report.
    static func generateReport(
        emails: [MBOXParser.RawEmail],
        title: String,
        investigatorName: String
    ) async -> Data {
        var aiInsights: String? = nil
        ...
        if #available(macOS 26, iOS 26, *) {
            aiInsights = await FoundationModelEngine.enhanceWithAI(
                scope: .investigation,
                emails: emails,
                context: "Generate insights for forensic investigation report"
            )
        }
        #endif
    ...
    static func generateReport(
        ...
        if let aiInsights = aiInsights, !aiInsights.isEmpty {
            finishAndStartNewPage(state: state)
            drawAIEnhancedAnalysis(state: state, insights: aiInsights)
        }
    ...
    private static func drawAIEnhancedAnalysis(
        state: DrawingState,
        insights: String
    ) {
        drawSectionHeader("AI-Enhanced Analysis", state: state)

        // Disclaimer: clearly label as AI-generated and non-deterministic
        let disclaimer = "NOTICE: The following insights were generated by Apple Intelligence (on-device foundation model). This content is AI-generated and non-deterministic -- results may vary between runs. It should be treated as supplementary analysis only and must not be relied upon as primary evidence. All preceding sections of this report use deterministic NLP methods that produce consistent, auditable results."
        let disclaimerAttrs: [NSAttributedString.Key: Any] = [
            .font: captionFont,
            .foregroundColor: chartRed
        ]
        drawText(disclaimer, attrs: disclaimerAttrs, state: state, maxWidth: contentWidth)
        state.currentY -= 8
        drawSeparator(state: state)
        state.currentY -= 4

        // Render the AI insights, splitting by paragraphs for proper pagination
        ...
        for paragraph in paragraphs {
            ...
            // Detect section headings from the AI output (lines that look like headers)
            if trimmed.hasPrefix("##") || trimmed.hasPrefix("**") || (trimmed.hasSuffix(":") && trimmed.count < 80) {
                state.currentY -= 4
                let cleanedHeading = trimmed
                    .replacingOccurrences(of: "##", with: "")
                    .replacingOccurrences(of: "**", with: "")
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                drawText(cleanedHeading, attrs: h2Attrs, state: state)
                state.currentY -= 2
            } else {
