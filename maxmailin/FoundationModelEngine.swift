...
struct FoundationModelEngine {
    ...
    private static func buildContext(from contextEmails: [MBOXParser.RawEmail], allEmails: [MBOXParser.RawEmail]? = nil) -> String {
        ...
        return boundedForModelContext(context)
    }

    /// Hard cap input strings before they enter `LanguageModelSession.respond(to:)`.
    /// Apple's on-device model has a 4096-token context window total
    /// (prompt + response). Budgeting ~512 tokens for instructions and
    /// ~512 for the response leaves ~3000 tokens for the input — at the
    /// model's ~4 chars/token average that's ~12,000 chars. The previous
    /// uncapped path was returning ~15,000 chars and intermittently
    /// blowing the window with
    /// "InferenceError::inferenceFailed::Context length of 4096 was
    /// exceeded during singleExtend".
    static let modelInputCharCap = 12_000

    static func boundedForModelContext(_ raw: String, maxChars: Int = modelInputCharCap) -> String {
        guard raw.count > maxChars else { return raw }
        // Truncate from the tail since the head (stats + thread summary)
        // is usually the most semantically dense. Add a clear suffix so
        // the model knows content was elided.
        let prefix = raw.prefix(maxChars - 80)
        return prefix + "\n\n[... context truncated to fit on-device model window]\n"
    }
