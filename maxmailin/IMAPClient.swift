...
final class IMAPClient: ObservableObject {
    ...
    // MARK: - STORE Commands (Write-Back)

    func markAsRead(messageNumbers: [Int]) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let ids = messageNumbers.map(String.init).joined(separator: ",")
        let response = try await sendCommand("STORE \(ids) +FLAGS (\\Seen)")
        guard response.contains("OK") else {
            throw IMAPError.commandFailed(command: "STORE +FLAGS \\Seen", response: response)
        }
        imapLog.info("Marked \(messageNumbers.count) messages as read")
    }

    func markAsUnread(messageNumbers: [Int]) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let ids = messageNumbers.map(String.init).joined(separator: ",")
        let response = try await sendCommand("STORE \(ids) -FLAGS (\\Seen)")
        guard response.contains("OK") else {
        ...
        imapLog.info("Marked \(messageNumbers.count) messages as unread")
    }

    func flagMessages(messageNumbers: [Int]) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let ids = messageNumbers.map(String.init).joined(separator: ",")
        let response = try await sendCommand("STORE \(ids) +FLAGS (\\Flagged)")
        guard response.contains("OK") else {
            throw IMAPError.commandFailed(command: "STORE +FLAGS \\Flagged", response: response)
        }
        imapLog.info("Flagged \(messageNumbers.count) messages")
    }

    func unflagMessages(messageNumbers: [Int]) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let ids = messageNumbers.map(String.init).joined(separator: ",")
        let response = try await sendCommand("STORE \(ids) -FLAGS (\\Flagged)")
        guard response.contains("OK") else {
    ...
    func deleteMessages(messageNumbers: [Int]) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let ids = messageNumbers.map(String.init).joined(separator: ",")
        let storeResp = try await sendCommand("STORE \(ids) +FLAGS (\\Deleted)")
        guard storeResp.contains("OK") else {
            throw IMAPError.commandFailed(command: "STORE +FLAGS \\Deleted", response: storeResp)
        }
        let expungeResp = try await sendCommand("EXPUNGE")
        guard expungeResp.contains("OK") else {
            throw IMAPError.commandFailed(command: "EXPUNGE", response: expungeResp)
        }
        imapLog.info("Deleted and expunged \(messageNumbers.count) messages")
    }

    func moveMessage(messageNumber: Int, toFolder: String) async throws {
        guard case .authenticated = connectionState else { throw IMAPError.notConnected }
        let escapedFolder = toFolder.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "\"", with: "\\\"")
        let copyResp = try await sendCommand("COPY \(messageNumber) \"\(escapedFolder)\"")
        guard copyResp.contains("OK") else {
            throw IMAPError.commandFailed(command: "COPY", response: copyResp)
        }
        try await deleteMessages(messageNumbers: [messageNumber])
        imapLog.info("Moved message \(messageNumber) to \(toFolder)")
    }

    // MARK: - Connection Retry

    func connectWithRetry(config: IMAPConfig, maxRetries: Int = 3) async throws {
        var lastError: Error?
        for attempt in 1...maxRetries {
            do {
                ...
                lastError = error
                imapLog.warning("Connection attempt \(attempt)/\(maxRetries) failed: \(error.localizedDescription)")
        ...
        throw lastError ?? IMAPError.connectionFailed("Failed after \(maxRetries) attempts")
    }

    // MARK: - Auto-Refresh (NOOP Polling)

    func startAutoRefresh(interval: TimeInterval = 120, onNewMail: @escaping () -> Void) -> Task<Void, Never> {
        Task { [weak self] in
            while !Task.isCancelled {
                try? await Task.sleep(nanoseconds: UInt64(interval * 1_000_000_000))
                guard !Task.isCancelled, let self else { return }
                guard case .authenticated = self.connectionState else { continue }
                do {
                    let response = try await self.sendCommand("NOOP")
                    if response.contains("EXISTS") {
                        onNewMail()
                    }
                } catch {
                    imapLog.warning("NOOP failed: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    static func saveCredentials(_ config: IMAPConfig) {
        UserDefaults.standard.set(config.server, forKey: "imap_server")
        UserDefaults.standard.set(Int(config.port), forKey: "imap_port")
        UserDefaults.standard.set(config.username, forKey: "imap_username")
        KeychainHelper.save(key: "imap_password", value: config.password)
    }

    static func loadSavedCredentials() -> IMAPConfig? {
        guard let server = UserDefaults.standard.string(forKey: "imap_server"),
              !server.isEmpty else { return nil }
        let port = UInt16(UserDefaults.standard.integer(forKey: "imap_port"))
        let username = UserDefaults.standard.string(forKey: "imap_username") ?? ""
        let password = KeychainHelper.load(key: "imap_password")
        guard !password.isEmpty else { return nil }
        return IMAPConfig(server: server, port: port == 0 ? 993 : port, username: username, password: password)
    }

    static func clearSavedCredentials() {
        UserDefaults.standard.removeObject(forKey: "imap_server")
        UserDefaults.standard.removeObject(forKey: "imap_port")
        UserDefaults.standard.removeObject(forKey: "imap_username")
        KeychainHelper.delete(key: "imap_password")
    }
