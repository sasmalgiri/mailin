...
struct LegalReviewWorkspaceView: View {
    ...
    private func runAISuggestions() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let msg: String = {
                let header = "Scan complete — \(total) emails reviewed. "
                if privilegedTotal == 0 {
                    return header + "No privileged communications found. All \(np) emails classified as Not Privileged."
                }
                var parts: [String] = []
                ...
                return header + "Found \(privilegedTotal) potentially privileged: \(parts.joined(separator: ", ")). Select an email to review and apply suggestions."
            }()
