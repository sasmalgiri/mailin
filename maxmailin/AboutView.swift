...
struct AboutView: View {
    ...
    @State private var swiftDataCount: Int? = nil
    ...
    @ViewBuilder
    private var swiftDataStatusBadge: some View {
        // Debug builds show a tappable badge that opens the v2 diagnostics
        // view. Release / App Store builds show a non-interactive status pill
        // so power users see that v2 storage is active, without exposing the
        // developer diagnostic panel.
        #if DEBUG
        NavigationLink(destination: SwiftDataDiagnosticsView()) {
            statusPill(interactive: true)
        }
        .buttonStyle(.plain)
        .task {
            swiftDataCount = (try? await EmailStore.shared.totalCount()) ?? 0
        }
        #else
        statusPill(interactive: false)
            .task {
                swiftDataCount = (try? await EmailStore.shared.totalCount()) ?? 0
            }
        #endif
    ...
    private func statusPill(interactive: Bool) -> some View {
        VStack(spacing: 2) {
            HStack(spacing: 6) {
                Image(systemName: "internaldrive").font(.caption)
            ...
            .padding(.vertical, 4)
            .background(Capsule().fill(statusColor.opacity(0.85)))

            if let count = swiftDataCount {
                let suffix = interactive ? " · tap to diagnose" : ""
                Text("\(count) emails in SwiftData store\(suffix)")
                    .font(.caption2)
                    .foregroundColor(AppColors.secondary)
            }
        }
    ...
    private var statusColor: Color {
        ...
        case .migrating, .checking: return .orange
